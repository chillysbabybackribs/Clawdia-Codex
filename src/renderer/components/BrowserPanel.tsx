...
function displayHostLabel(url: string): string {
  try {
...
function BrowserTabBg({ active, hovered }: { active: boolean; hovered: boolean }) {
  const fill = active ? '#1a1a1a' : hovered ? '#141414' : '#0c0c0c';
  const stroke = active
...
export default function BrowserPanel({ reservedRight = 0 }: { reservedRight?: number }) {
  ...
  const activeSiteLabel = activeTab && !isNewTab ? displayHostLabel(activeTab.url) : 'New tab';
  ...
  useEffect(() => {
    ...
    const fullApi = (window as any).clawdia;
    if (fullApi?.onMirrorNavigate) {
      cleanups.push(fullApi.onMirrorNavigate(() => setMirrorActive(true)));
    }
    if (fullApi?.onMirrorDone) {
      cleanups.push(fullApi.onMirrorDone(() => setMirrorActive(false)));
    }
  ...
  const handleNavigate = useCallback((e: React.FormEvent) => {
    ...
    setUrlInput(displayed);
    pendingUrlRef.current = displayed;
  ...
  return (
    <div className="browser-shell relative flex h-full flex-col bg-surface-0">
      <BrowserTabSvgDefs />
      {/* Chrome-style browser tab strip */}
      <div
        className="browser-top-chrome drag-region flex items-end px-2 pt-[8px] flex-shrink-0 overflow-hidden"
        style={{ height: 44, position: 'relative' }}
      >
        <div className="browser-tabstrip flex items-end min-w-0 overflow-x-auto no-scrollbar">
          {tabs.map((tab, index) => {
                ...
                >
                  ...
                  {tabs.length > 1 && (
                    <button
                      ...
                      style={{ color: tab.isActive || isHovered ? 'rgba(255,255,255,0.4)' : 'transparent' }}
                    >
      ...
      <div className="browser-toolbar flex items-center gap-2 px-3 h-[48px] flex-shrink-0">
        <div className="browser-nav-cluster flex items-center gap-0.5">
          <button onClick={handleBack} className="flex items-center justify-center w-8 h-8 rounded-full text-text-muted hover:text-text-secondary hover:bg-white/[0.05] transition-colors cursor-pointer">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
          </button>
          <button onClick={handleForward} className="flex items-center justify-center w-8 h-8 rounded-full text-text-muted hover:text-text-secondary hover:bg-white/[0.05] transition-colors cursor-pointer">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
          </button>
          <button onClick={handleRefresh} className="flex items-center justify-center w-8 h-8 rounded-full text-text-muted hover:text-text-secondary hover:bg-white/[0.05] transition-colors cursor-pointer">
            {isLoading ? (
        ...
        <form onSubmit={handleNavigate} className="flex-1">
          <div className="browser-omnibox relative">
            {isLoading && (
              <div className="absolute bottom-0 left-0 h-[2px] bg-white/50 rounded-full animate-pulse-soft" style={{ width: '60%' }} />
            )}
            {ghostSuffix && (
              <div className="absolute inset-0 flex items-center px-10 pointer-events-none font-mono text-xs">
                <span className="invisible whitespace-pre">{urlInput}</span>
                <span className="text-text-tertiary whitespace-pre">{ghostSuffix}</span>
              </div>
            )}
            <div className="browser-omnibox-icon absolute left-3 top-1/2 -translate-y-1/2 text-white/45 pointer-events-none">
              {isNewTab ? (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <input
              ...
              onFocus={handleFocus}
              onBlur={handleBlur}
              className={`w-full h-[34px] bg-transparent text-text-secondary text-xs pl-10 pr-28 rounded-full border border-transparent hover:border-white/[0.05] focus:border-white/[0.14] focus:text-text-primary outline-none transition-all font-mono${isNewTab ? ' shadow-[0_0_0_2px_rgba(255,255,255,0.08)]' : ''}`}
              placeholder="Enter URL or search..."
              autoComplete="off"
              spellCheck={false}
            />
            {!isFocused && (
              <div className="browser-omnibox-site absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">
                {activeSiteLabel}
              </div>
            )}
          </div>
        ...
        <button className="browser-action no-drag flex items-center justify-center w-8 h-8 rounded-full text-text-muted hover:text-text-secondary hover:bg-white/[0.05] transition-colors cursor-pointer" title="Bookmark">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="m19 21-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" />
          </svg>
        </button>
        <button className="browser-action no-drag flex items-center justify-center w-8 h-8 rounded-full text-text-muted hover:text-text-secondary hover:bg-white/[0.05] transition-colors cursor-pointer" title="More">
      ...
      <div ref={viewportRef} className="browser-viewport relative flex-1 bg-surface-0">
        {!activeTab && (
